<template>
  <article class="tfs-article">
    <h1 v-if="!fileUploaded">{{ appData.name }}</h1>
    <hr v-if="!fileUploaded">
    <Alert type="success" v-if="fileUploaded" class="mt20">
      文件已经上传成功，请耐心等待服务器解析包信息，您也可以点击下方的重新上传按钮重新上传文件。
    </Alert>
    <Row v-if="fileUploaded" class="tfs-app-mata-row">
      <Col span="5">
      ...
      <Col span="12" style="line-height: 26px;">
        <Row>{{ appData.name }}</Row>
        <Row>ID: {{ appMetaData.package }}</Row>
        <Row>大小: {{ formData.fileSize }}</Row>
        <Row>版本: {{ appMetaData.versionName }}</Row>
      </Col>
    ...
    <div class="tfs-form-wrapper">
      <Form :model="formData" :label-width="80">
        ...
        <Form-item label="默认下载:" v-if="fileUploaded">
...
export default {
  async asyncData(ctx) {
    ...
    const { data } = await $axios.$get(`app?appId=${query.appId}`);

    return { appData: data[0], appId: query.appId };
  },
  middleware: 'auth',
  computed: {
  ...
  methods: {
    ...
    async _getAppPackMeta(pkgHashId, appId, pkgFileName) {
      const { data } = await this.$axios.$put('app', { appId, pkgHashId, pkgFileName });
    ...
    async handleSubmit(app) {
      if (this.appMetaData !== {} && this.formData.fileFingerPrint !== '') {
        ...
        await this.$axios.$post('app/file', {
          ...
          type: 'apk',
          appId: this.appId,
          downloadUrl: `http://localhost:1080/files/${this.formData.fileFingerPrint}`,
    ...
    handleFileUpload(file) {
      ...
      uploader = new tus.Upload(file, {
        ...
        onSuccess: async function() {
          ...
          ctx.loading = true
          await ctx._getAppPackMeta(ctx.formData.fileFingerPrint, ctx.appId, ctx.formData.fileName);
          ctx.loading = false
        }
      })
      uploader.start()
